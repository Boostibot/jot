...
namespace jot
{
    ...
    struct Ring_Allocator : Allocator
    {
        ...
        Allocation_Result allocate(isize size, isize align) noexcept override 
        {
            ...
            u8 is_allowed = aligned_size > MAX_ALLOWED_SIZE;
        ...
        Allocator_State_Type deallocate(Slice<u8> allocated, isize align) noexcept override 
        {
            ...
            while (true) {
                ...
                //assert(first_slot->size <= jot::memory_constants::KIBI_BYTE / sizeof(Slot) * 2);
    ...
    struct Intrusive_Stack_Resize : Allocator
    {
        ...
        Allocation_Result allocate(isize size, isize align) noexcept override 
        {
            ...
            isize stub_size = ptrdiff(slot, stub) - sizeof(Slot);
            isize slot_offset = stub_size;
            isize stub_offset = ptrdiff(stub, last_block_from);

            u32 reduced_slot_size = cast(u32) slot_size / sizeof(Slot);
            u32 reduced_stub_size = cast(u32) stub_size / sizeof(Slot);
            u32 reduced_slot_offset = reduced_stub_size;
            u32 reduced_stub_offset = cast(u32) stub_offset / sizeof(Slot);
        ...
        bool check_allocated(Slice<u8> allocated, isize align) noexcept
        {
            ...
            bool sizes_match = slot_size * sizeof(Slot) >= cast(u64) aligned_size;
            assert(sizes_match);
        ...
        Allocator_State_Type deallocate(Slice<u8> allocated, isize align) noexcept override 
        {
            ...
            while (true) 
            {
                ...
                last_block_from = (cast(u8*) last_slot) - last_slot->prev_offset * sizeof(Slot);
                last_block_to = cast(u8*) last_slot;
        ...
        Allocation_Result resize(Slice<u8> allocated, isize align, isize new_size) noexcept override 
        {
            ...
            while (true) 
            {
                ...
                if(cast(u8*) next_slot >= last_block_to)
                {
                    ...
                    isize new_reduced_size = ptrdiff(aligned_end, allocated.data) / sizeof(Slot);
    ...
    struct Stack_Ring : Allocator
    {
        ...
        u8* current_end = nullptr;
        bool is_inside_out = false;
        ...
        Allocation_Result allocate(isize size, isize align) noexcept override 
        {
            ...
            u8* last_alloc_to = last_block_to;
            //get the first adress at which it would be possible to place the slot header
            u8* available_from = last_alloc_to + sizeof(Slot);
            ...
            //If past end
            if(aligned_to > current_end) [[unlikely]]
            {
                //mark the remainding size with slot (but unused)
                Slot* fill_rest = cast(Slot*) last_block_to;
                isize fill_size = ptrdiff(buffer_to, fill_rest);
                fill_rest->size = cast(u32) (fill_size / SIZE_MULT);

                //recalc the ptrs
                last_alloc_to = buffer_from;
                available_from = cast(u8*) (last_alloc_to + 1);
                aligned_from = align_forward(available_from, align);
                aligned_to = align_forward(aligned_from + size, alignof(Slot));
            } 
            ...
            // When the alignment is <= sizeof(Slot) the adress of stub == address of slot
            //  so the stub gets completely overwritten
            Slot* stub = cast(Slot*) last_alloc_to;
            Slot* slot = (cast(Slot*) aligned_from) - 1;
            isize slot_size = ptrdiff(aligned_to, aligned_from);
            isize stub_size = ptrdiff(slot, stub) - sizeof(Slot); //only the size of data in between headers => - sizeof(Slot)
            isize slot_offset = ptrdiff(slot, last_block_from);
            ...
            //If is too big we use parent
            u8 is_too_big  = reduced_slot_size >= USED_BIT;
            u8 is_past_end = aligned_to > current_end;
            if(is_too_big | is_past_end) [[unlikely]]
                return parent->allocate(size, align);
