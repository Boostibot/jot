...
namespace jot
{
    ...
    namespace bucket_array_internal
    {
        ...
        static 
        uint32_t add_free_buckets(Untyped_Bucket_Array* bucket_array, Bucket_Array_Growth growth, isize slot_size, isize slots_align) 
        {
            ...
            if(add_bucket_block(bucket_array, new_size, slot_size, slots_align) == false)
            {
                PANIC(panic_cformat, "bucket array allocation failed! "
                    ...
                    "bucket array: {used: %t, capacity %t, buckets %t, bucket_size: %t}", 
                    new_size, slot_size, slots_align,
                    bucket_array->_total_used, bucket_array->_total_capacity, size(bucket_array->_buckets), BUCKET_SIZE);
            }
    ...
    isize from_bucket_index(Bucket_Index index)
    {
        ...
        assert(0 <= index.slot_i && index.slot_i < bucket_array_internal::BUCKET_SIZE && "must be within range!");
